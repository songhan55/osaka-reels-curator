...
// Fetch all groups and reels from Supabase Cloud DB
export async function fetchGroupsFromDb() {
  if (!isSupabaseConfigured || !supabase) return DEFAULT_GROUPS;
  ...
  try {
    ...
    return groups.map(g => {
      const groupReels = (reels || []).filter(r => r.group_id === g.id);
